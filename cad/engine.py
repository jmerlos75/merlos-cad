...
class CADWindow:
    ...
    def _ltscale_dialog(self):
        ...
        scales = [
            ("1:1",    1.0,   1.0),   ("1:2",   0.5,   2.0),
            ("1:5",    0.2,   5.0),   ("1:10",  0.1,  10.0),
            ("1:20",  0.05,  20.0),   ("1:25",  0.04,  25.0),
            ("1:50",  0.02,  50.0),   ("1:100", 0.01, 100.0),
            ("1:200",0.005, 200.0),   ("1:500",0.002, 500.0),
            ("2:1",   2.0,   0.5),    ("5:1",   5.0,   0.2),
        ]
    ...
    def _open_native_dialog(self, fn, **kw):
        """Wrapper para filedialog.askXxx."""
        return fn(**kw)

    def _leer_config_ia(self) -> dict:
        """Lee settings.json con cache por mtime."""
        import json as _j
        path = self._cfg_path()
        try:
            mtime = os.path.getmtime(path)
            cache = getattr(self, '_cfg_ia_cache', None)
            if cache and cache[0] == mtime:
                return cache[1]
            with open(path, 'r', encoding='utf-8') as f:
                data = _j.load(f)
            self._cfg_ia_cache = (mtime, data)
            return data
        except Exception:
            cached = getattr(self, '_cfg_ia_cache', None)
