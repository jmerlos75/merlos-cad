# build.ps1 — Build Merlos CAD .exe (Windows)
#
# Uso:
...
#
# Prerequisitos:
#   pip install pyinstaller customtkinter ezdxf Pillow
#
...
Write-Host "=== Merlos CAD — PyInstaller build ===" -ForegroundColor Cyan

# 1. Smoke test del entry point (import only, sin lanzar UI)
Write-Host "`n[1/3] Verificando imports..." -ForegroundColor Yellow
$check = python -c "import sys; sys.path.insert(0,'$REPO'); from cad.engine import CADWindow; print('OK')" 2>&1
if ($LASTEXITCODE -ne 0) {
    Write-Host "ERROR: import fallido — $check" -ForegroundColor Red
    exit 1
}
Write-Host "      Imports: $check"
...
# 3. Post-build: copiar docs a la raíz del bundle (PyInstaller 6 los pone en _internal/)
